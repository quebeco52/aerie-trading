...
class SteelManufacturingBusinessModel extends StandardCorporateBusinessModel
{
    ...
    public function getCapexCyclicality(): float { return 0.8; } // Massive fixed infrastructure
    
    public function getSurpriseBlendWeights(): array { return ['eps_weight' => 0.40, 'revenue_weight' => 0.60]; }
    ...
    protected function calculateSectorPhysics(Stock $stock, float $expectedRevenue, float $realizedVariableMargin, float $fixedCosts, float $baselineVol, \App\DTO\MacroStateDTO $macroState, MathUtility $mathUtility): SectorPhysicsResult
    {
        ...
        $energyDrag = max(0.0, ($macroState->energyPriceIndexEma - MacroEngine::ENERGY_BASELINE) / 100.0) * 1.5; // Blast furnaces use massive energy

        $domesticZ = $mathUtility->generatePersistentZ($momentum['domestic_production'] ?? 0.0, 0.25) + $macroBoost - $energyDrag;
        $exportZ = $mathUtility->generatePersistentZ($momentum['export_dumping'] ?? 0.0, 0.10) + ($macroBoost * 1.5); 

        $domesticRevenue = $expectedRevenue * self::DOMESTIC_WEIGHT * (1.0 + ($domesticZ * ($baselineVol * self::DOMESTIC_VARIANCE_SCALAR)));
        $exportRevenue = $expectedRevenue * self::EXPORT_WEIGHT * (1.0 + ($exportZ * ($baselineVol * self::EXPORT_VARIANCE_SCALAR)));
        ...
        return new SectorPhysicsResult(
            ...
            isPublicEvent: null,
            streamZ: [
                'domestic_production' => $domesticZ,
                'export_dumping'      => $exportZ,
            ],
            streamRevenue: [
