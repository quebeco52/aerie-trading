 ...
 *
 * Seven fields change on every name on every tick: the price, the instantaneous volatility, the impact
 * variance, the momentum trend, the credit spread, the corporate flow backlog, and the incumbent CEO's
 * tenure — that last because the succession hazard ages every board on every tick. Because they always
 * change, every stock was a dirty entity at every flush, and Doctrine sends one UPDATE per dirty entity —
 * sixty-odd synchronous round trips inside the tick transaction, several times a second, for the same seven
 ...
 * whose only changes are these seven is not written at all. Whatever flushes the working set must write them
