...
## 1. AI Policy & Behavioral Constraints
...
- The project is containerized. Typical developer commands include `make install`, `make ticker` (for the live market ticker), and `make tailwind-watch` (for frontend styling).
- When assisting with frontend changes, utilize Tailwind CSS utility classes and ensure a responsive, dynamic design. Use modern styling practices.

## 6. Real-Time Interactions
- Real-time features (like live price updates) are broadcast via WebSocket (Workerman) and Redis. Ensure that any new live data metrics hook into this existing infrastructure efficiently without degrading performance.
